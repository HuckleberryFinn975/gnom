...
class MainClass:
	...
	token1, token2, to1 = tgData["pushToken"], tgData["followToken"], 5
	...
	def killWindow(self):
		...
		try:
			print("Killing OASIS")
			oasis = ahk.find_window(title = f'{data["titles"][self.race]}')
			if oasis:
				...
				self.send_message(f'    AHK doesnt see {data["titles"][self.race]}', self.token2)
		except Exception as ex:
			print(ex)
	def setWindow(self):
		ahk = AHK()
